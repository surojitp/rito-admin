...
@Component({
    selector: 'pharmacylocatorcontent',
    styleUrls: ['./pharmacylocatorcontent.component.css'],
    templateUrl: './pharmacylocatorcontent.component.html',
})
// class Select
export class PharmacylocatorcontentComponent implements OnInit {
    edit: boolean;
    data: any;
    gallery_list: any;
    admintoken: any;
    ...
    ngOnInit(): void {
        ...
        var postdata = {
            page_type: 'pharmacy_locator'
        };
        this._appservice.getAllContent(postdata).subscribe((Response) => {
    ...
    getDetails(str: any) {
        this.edit = true;
        this.popupDiv = true;
        this.pagetitle = 'Edit Home Content';

        this._appservice.getDetailsContent(str).subscribe((Response) => {
            if (Response.STATUSCODE == 4002) {
    ...
    saveData() {
        ...
        if (this.edit == true && flag == 0) {
            if (this.item.title == '' || this.item.title == undefined) {
                ...
                this._appservice.editContent(this.item, this.uploadfile)
                    .subscribe((Response) => {
                        if (Response.STATUSCODE == 4002) {
                            this._message.showError(Response.response_message);
                            localStorage.clear();
