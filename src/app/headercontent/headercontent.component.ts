...
export class HeadercontentComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this._appservice.getAllHeader().subscribe((Response) => {
    ...
    getDetails(str: any) {
        this.edit = true;
        this.popupDiv = true;
        this.pagetitle = 'Edit Header Content';
        this._appservice.getDetailsHeader(str).subscribe((Response) => {
            if (Response.STATUSCODE == 4002) {
                this._message.showError(Response.message);
                localStorage.clear();
                location.reload();
    ...
    saveData() {
        ...
        if (this.edit == true && flag == 0) {
            if (this.item.title == '' || this.item.title == undefined) {
                ...
                this._appservice.editHeader(this.item, this.uploadfile)
                    .subscribe((Response) => {
                        if (Response.success) {
